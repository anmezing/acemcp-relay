...
func TestClassifyIndexFailure(t *testing.T) {
	cases := []struct {
		...
		{"embedding input", indexJobStatusFailed, "Embedding API 错误: HTTP 400: ensure input is encoded in valid UTF-8 format and special characters are properly escaped", indexFailureDiagnostic{"embedding_input_rejected", "provider", "fix_embedding_input"}},
		...
		{"file count", indexJobStatusFailed, "manifest exceeds configured file count limit", indexFailureDiagnostic{"repository_file_limit", "client", "reduce_repository"}},
		{"file size", indexJobStatusFailed, "file exceeds the configured byte limit: generated/data.json", indexFailureDiagnostic{"repository_file_size_limit", "client", "reduce_repository"}},

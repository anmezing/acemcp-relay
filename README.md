...
| `DAILY_INDEX_BYTES_LIMIT` | 每用户每日索引字节上限（Asia/Shanghai 自然日），超限返回 429；`0` 不限。索引通道**不能**靠请求数配额约束：创建一次 job 只计 1 次请求，之后的 `/relay/remote-index` 批次全部豁免（一次扫描上千批，按请求计费会误伤正常用户），而 embedding 成本全在那些批次上——无此上限时 1 次请求配额可驱动上百 GB 的 embedding 调用。单文件与单批原始内容上限均为 512 KiB；Relay 还会校验转发给 LCE 的完整 JSON-RPC 请求体不超过 4 MiB | `2147483648`（2 GiB） |
